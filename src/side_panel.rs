...
impl<const EXTRA_REGS: usize> SidePanel<EXTRA_REGS> {
  ...
  fn print_anr(&mut self, anr: &hp_anr::HP_AnR) {
  ...
  fn print_datastorage(&mut self, ram: &hp_ram::HP_RAM<EXTRA_REGS>) {
  ...
  fn print_cnt(&self, cnt: &hp_cnt::HP_CnT) {
  ...
  fn print_status(&mut self, cnt: &hp_cnt::HP_CnT) {
